...
class AMPA_spine
{
 public:
    ...
    static constexpr double a_r=1.1;   // per mM per ms, alpha rise rate of syn conductance
                                   // 1.1e6 per M per second
    static constexpr double a_d=0.67;  // per ms, alpha decay rate of syn conductance
                                   // original 0.67;
    
    static constexpr double gAMPA = 0.35e-6;// max conductance of AMPAR at single synapse, mS
                                        // range is 0.35 to 1.0 nS, p10 Destexhe
    static constexpr double Eampa=0;                                   
    /*   
...
class Kainate_spine {
    ...
    //   units are mM  and  ms        
    static constexpr double Tmax=1,      Kp=5;
    static constexpr double a_r=1.1,     a_d=0.01;   
    static constexpr double Vkainate=0,  gKainate=0.9e-6;  // mS
...
{    
    ...
    static constexpr double a_r=0.072;   // rise and decays times per mM per ms
    static constexpr double a_d=0.0066;      
    static constexpr double Enmda=0;       // Destexhe 1998: max gNMDA = 0.01 .. 0.6 nS per syn
    static constexpr double gNMDA=0.7e-6;  // conductance mS;   0.7 nS from Traub 1994, p382, excitatory neuron
    static constexpr double Mg=1.0;        // conc of extracellular Mg in mM
...
class GABA_A_spine {
    ...
    public:
    //   units are mM  and  ms    
    static constexpr double a_r = 5;  
    static constexpr double a_d = 0.18;  
    static constexpr double V_GABA=-75;   //   reversal potential for Cl- 
    static constexpr double g_GABA=0.5e-6;   // mS,  Destexhe 1998: max g_gaba_a = 0.25 .. 1.2 nS per syn
...
class GABA_B_spine
{
    ...
    static constexpr double V_T = 2;          
    static constexpr double Kp  = 5;
    static constexpr double a_r = 0.09;  
    static constexpr double a_d = 0.0012; 
    static constexpr double E_K =-95;    //   -90 ... -105, Ermentrout, FMN p163
    static constexpr double n   = 4;  
    static constexpr double Kd  = 100; 
    static constexpr double K3  = 0.18;  
    static constexpr double K4  = 0.034; 
    static constexpr double g_GABA=1.0e-6;   // mS   Destexhe 1998: max g_gaba_b = 1.0 nS per syn
