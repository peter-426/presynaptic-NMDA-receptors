...
{    
    ...
    */
    ...
    static constexpr double a_r=0.072;     // p163, Ermentrout 0.072 /mM/ms, in mM but here using uM !
                                       // a:alpha, larger a_r, a_d means faster rise, decay
    static constexpr double a_d=0.0066;    // p163, Ermentrout 0.0066 /ms
                                       // Destexhe 1998: max gNMDA = 0.01 .. 0.6 nano Siemens per synapse
    static constexpr double Vnmda=0;   
    static constexpr double gNMDA=0.3;    //  .21    0.7 * 10; 
                                      //  max conductance=0.7 for excitatory neuron, p382 of Traub 1994, 
    static constexpr double Mg=2.0;       //  extracellular [Mg++] in mM
...
public:
    ...
    //
    static constexpr double v_half=-17;       // Half-activation voltage for wild type mice; mV; Ishikawa (2005)
    static constexpr double slope_factor=8.4; // Slope factor for wild type mice; mV; Ishikawa (2005)
    
    static constexpr double tau_mc=10;       // Tewari 2012b, the LTP paper;   Ishikawa et al. 2005 
    ...
    static constexpr double g_ca=3.3;    // 3.3 pS;  conductance of single Ca_v_2.1 at calyx,  Sheng (2012) 
                                     // 
    ...
    static constexpr double rho_ca=1;  // number of channels in cluster was 32 and 0.03 for surface area in bouton.h
