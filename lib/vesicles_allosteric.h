...
static constexpr double kon=1e5;   // 1e5 /M /ms;  1e8 /M /s 
static constexpr double koff=4;      // 4      /ms;      4000 /s

static constexpr double b=0.25; // cooperativity factor;  b=0.5 in Lou (2005)

static constexpr double f=31.3;  // multiplier constexprant to I, representing the 
                             // greater rate of fusion from faster states

static constexpr double I=2e-7; // the instantaneous rate of fusion per ms, 
                            // representing the constexprant exocytosis of
...
static constexpr double nv=2;    // Number of vesicles; nv=2 in Nikonenko & Skibo (2006)
static constexpr double gv=60;   // Glutamate concentration in single vesicle;  mM;   Montana et al. (2006)
static constexpr double degG=10; // Extracellular glutamate degradation rate; per ms; Destexhe et al (1998)

// Vesicle Regulatory time constexprants ms,  Tsodyks & Markram (1998)
static constexpr double tau_rec=800;  // Vesicle recovery time constexprant;  ms;   Tsodyks & Markram (1997)
static constexpr double tau_inact=3;  // Vesicle inactivation time constexprant;  ms;   Tsodyks & Markram (1997)
